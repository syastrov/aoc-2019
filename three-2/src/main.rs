...
impl Wire {
    fn new(input: String, id: WireId) -> Self {
        Wire {
            id: id,
...
struct Grid {
    cells: HashMap<(Value, Value), HashMap<WireId, NumSteps>>
}
...
impl Grid {
    ...
    fn set(&mut self, x: Value, y: Value, wire_id: WireId, num_steps: NumSteps) {
        let cell = self.cells.entry((x, y)).or_insert_with(|| HashMap::new());
        let cell_value = &mut *cell;
        // Insert num steps only the first time.
        // If the wire crosses the cell again, we are only interested in the first time.
        match cell_value.get(&wire_id) {
            Some(_) => {}
            None => {
                cell_value.insert(wire_id, num_steps);
            }
        };
        //cell_value.entry(wire_id).or_insert(num_steps);
    }
    fn get_total_num_steps(&mut self, x: Value, y: Value) -> NumSteps {
        let cell = self.cells.get(&(x, y));
        cell.unwrap().values().sum()
    }
    fn fill_grid(&mut self, wire: Wire) {
        let mut steps: NumSteps = 0;
        let mut cur_pos: (Value, Value) = (0, 0);
        for Movement { direction, amount } in wire.movements {
            let delta: (Value, Value) = match direction {
                Direction::UP => (0, 1),
                Direction::DOWN => (0, -1),
                Direction::RIGHT => (1, 0),
                Direction::LEFT => (-1, 0),
            ...
            for _ in 1..amount + 1 {
                ...
                self.set(cur_pos.0, cur_pos.1, wire.id, steps);
            }
        }
    }
...
fn main() {
//    let wire1_str: String = "U7,R6,D4,L4".to_string();
//    let wire2_str: String = "R8,U5,L5,D3".to_string();

//  let wire1_str: String = "R75,D30,R83,U83,L12,D49,R71,U7,L72".to_string();
//  let wire2_str: String = "U62,R66,U55,R34,D71,R55,D58,R83".to_string();
    ...
    let mut steps: Vec<NumSteps> = intersections.iter().map(|(x, y)| grid.get_total_num_steps(*x, *y)).collect();
    steps.sort();
//    println!("{:?}", steps);
    println!("{:?}", steps[0]);
}
